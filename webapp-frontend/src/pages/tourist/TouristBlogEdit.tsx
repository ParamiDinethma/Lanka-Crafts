...
export function TouristBlogEdit() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await getBlogs(1, 'recent'); // naive approach; optimally we'd fetch by ID
        const myBlog = res.data.blogs?.find((b: any) => b._id === id);
        if (myBlog) {
          if (myBlog.author._id !== tourist?.id) {
            ...
            <div className="space-y-6">
              ...
              <div className="pt-6 border-t flex flex-col sm:flex-row gap-4 justify-between items-center">
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button onClick={() => handlePublish()} disabled={submitting || deleting} className="px-8 py-3 rounded-xl bg-[#C1440E] text-white font-semibold hover:bg-[#A33A0C]">
                    {submitting ? 'Saving...' : 'Update Story'}
                  </button>
                </div>
